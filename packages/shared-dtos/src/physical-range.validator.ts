...
export interface TechnicalParametersDto {
  // 1. Sólidos disueltos del agua de entrada (TDS Entrada - ppm)
  inletTdsPpm: number;

  // 2. Sólidos disueltos post membrana (TDS Post-Membrana - ppm)
  postMembraneTdsPpm: number;

  // 3. Sólidos disueltos de producto final (TDS Producto Final / Permeado - ppm)
  finalProductTdsPpm: number;

  // 4. pH
  ph: number;

  // 5. Conductividad (µS/cm)
  conductivityUs: number;

  // 6. Resistividad (MΩ·cm)
  resistivityMohm: number;

  // 7. Litros consumidos de la red (Agua cruda de entrada)
  networkConsumedLiters: number;

  // 8. Litros totales de agua pura (Agua tratada/permeado)
...
export function validatePhysicalParameters(params: Partial<TechnicalParametersDto>): PhysicalRangeValidationResult {
  ...
  if (
    params.networkConsumedLiters != null &&
    params.totalPureWaterLiters != null &&
    params.networkConsumedLiters > 0 &&
    params.totalPureWaterLiters > params.networkConsumedLiters
  ) {
    anomalies.push(
      `Inconsistencia volumétrica: el agua pura producida (${params.totalPureWaterLiters} L) no puede superar el agua consumida de la red (${params.networkConsumedLiters} L).`
    );
  }
